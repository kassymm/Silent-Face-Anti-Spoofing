...
# base_directory = '/Users/kassymmukhanbetiyar/Development/Verigram/CelebA/archive/CelebA_Spoof_/CelebA_Spoof'

# total_rows = len(df)
# progress_interval = 250
# processed_count = 0
# checkpoint_file = 'dataset2.csv'
...
#         elapsed_time = time.time() - last_interval_start_time
#         last_interval_start_time = time.time()
#         average_time_per_sample = elapsed_time / progress_interval
...
#         # Save the DataFrame as a checkpoint (append mode)
#         df.iloc[processed_count - progress_interval:processed_count].to_csv(checkpoint_file, mode='a', header=False)
...
for row_name in tqdm(df.index[processed_count:], total=total_rows - processed_count):
    image_path = os.path.join(base_directory, row_name)
    label, value = test(image_path)
    df.loc[row_name, 'Real'] = label
    df.loc[row_name, 'Prediction Score'] = value
